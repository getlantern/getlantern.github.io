...
Admittedly it’s not a [big language](https://developers.google.com/protocol-buffers/docs/proto3), but its language guide is probably 1/4 the size of [Go](https://golang.org/ref/spec), which makes it non-trivial in my book. 
...
In this example, we implement a [server-streaming](http://www.grpc.io/docs/guides/concepts.html#server-streaming-rpc) service that takes a query and returns a stream of rows.
...
- We send two different types of datum to the stream, first the result from `db.SQLQuery` and then individual `Row`s.  This is possible because streams are untyped.  This could have been accomplished using an `Any` type in Protocol Buffers as well.

- The result from `db.SQLQuery` is part of the application domain, but we can happily send it along since MessagePack will serialize whatever we throw at it.  Note however that we do remove a couple of things that we don’t want to send to the client.

- `Row` is actually a data-transfer object created specifically for this API.  That’s because it works better for the client than sending the Entries from which the Rows are made.
...
- We use `grpc.NewClientStream` to get a stub for the service.  Note that this refers directly to the stream defined in the service descriptor.

- Note that we also have to specify the method as `/<ServiceName>/<StreamName>`, i.e. `/TibsDB/Query`.

- Once we have a stream, we can send and receive messages to/from it.